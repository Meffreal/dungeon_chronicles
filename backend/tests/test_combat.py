...
def _player(hp=200, atk=25, def_=10, spd=12, luck=5, level=5):
    return CombatantConfig(name="Hrdina", hp=hp, atk=atk, def_=def_, spd=spd, luck=luck, level=level, cls="", mp=0)


def _enemy(hp=80, atk=12, def_=5, spd=6, luck=3, level=3):
    return CombatantConfig(name="Skřet", hp=hp, atk=atk, def_=def_, spd=spd, luck=luck, level=level, cls="", mp=0)
...
def test_dodge_chance_minimum():
    assert _dodge_chance(defender_spd=1, attacker_spd=50) == pytest.approx(0.03)


def test_dodge_chance_fast_defender():
    slow = _dodge_chance(defender_spd=10, attacker_spd=10)
    fast = _dodge_chance(defender_spd=20, attacker_spd=10)
    assert fast > slow


def test_dodge_chance_maximum():
    chance = _dodge_chance(defender_spd=100, attacker_spd=1)
...
def test_calc_damage_crit_multiplier():
    with patch("game.combat_engine.random") as mock_rng:
        mock_rng.uniform.return_value = 1.0
        normal = _calc_damage(atk=20, def_=0, is_crit=False)
        crit   = _calc_damage(atk=20, def_=0, is_crit=True)
    assert normal == 20
    assert crit == int(20 * 1.75)   # 35


def test_calc_damage_minimum_one():
    with patch("game.combat_engine.random") as mock_rng:
        mock_rng.uniform.return_value = 0.85
        dmg = _calc_damage(atk=1, def_=9999, is_crit=False)
    assert dmg >= 1
...
def test_simulate_combat_player_wins():
    player = _player(hp=1000, atk=200, def_=100, spd=20)
    enemy  = _enemy(hp=5, atk=1, def_=0, spd=1)
...
def test_simulate_combat_enemy_wins():
    player = _player(hp=5, atk=1, def_=0, spd=1)
    enemy  = _enemy(hp=1000, atk=200, def_=100, spd=20)
    result = simulate_unified_combat(player, enemy)
...
def test_simulate_combat_timeout_resolved_by_hp():
    ...
    player = _player(hp=1000, atk=1, def_=9999)
    enemy  = _enemy(hp=100, atk=1, def_=9999)
...
def test_calculate_win_chance_strong_player():
    strong = _player(hp=1000, atk=200, def_=100, spd=50)
    weak   = _enemy(hp=10, atk=1, def_=0, spd=1)
    assert calculate_win_chance(strong, weak) >= 0.9


def test_calculate_win_chance_weak_player():
    weak   = _player(hp=10, atk=1, def_=0, spd=1)
    strong = _enemy(hp=1000, atk=200, def_=100, spd=50)
