// ── DUNGEONS WORLD MAP ────────────────────────────────────────────────────────
// SVG fantasy mapa se 3 zónami. Klik na zónu → boss list (dungeon.js).
...
async function loadDungeons() {
  ...
  try {
    const [dungeonCfg, bossData] = await Promise.all([
      api('GET', '/dungeon/config'),
      api('GET', '/dungeon/boss/list'),
    ]);
    DUNGEON_DATA = Array.isArray(dungeonCfg) ? dungeonCfg : [];

    _bossProgressMap = {};
    for (const d of (bossData.dungeons || [])) {
      _bossProgressMap[d.key] = d;
    }

    renderDungeons(bossData);
  } catch (e) {
...
function renderDungeons(bossData) {
  const el = document.getElementById('dungeon-content');
  if (!el) return;

  const { on_cooldown, cd_remaining } = bossData;
  ...
  el.innerHTML = `
    ...
    <div class="dng-map-container">
      ...
      <div class="dng-map-hud">
        <span class="dng-map-hud-stat"><span class="c-purple">${DUNGEON_DATA.length}</span> zón</span>
        <span class="dng-map-hud-sep">·</span>
        <span class="dng-map-hud-stat">50 bossů / zóna</span>
      </div>
      <div class="dng-map-nodes">
        ${DUNGEON_DATA.map(d => _renderMapNode(d, _bossProgressMap[d.dungeon_key] || {})).join('')}
      </div>
...
function _renderMapNode(d, prog) {
  const pos        = MAP_POSITIONS[d.dungeon_key] || { left: '50%', top: '50%' };
  const isUnlocked = prog.is_unlocked !== false;
  const isComplete = (prog.highest_boss || 0) >= 50;
  const lockedCls  = isUnlocked ? '' : 'locked';
  const completeCls = isComplete ? 'complete' : '';
  const progressPct = prog.progress_pct || 0;

  let nodeIcon, statusText;
  if (isComplete) {
    nodeIcon   = '✅';
    statusText = 'Dokončeno';
  } else if (isUnlocked) {
    nodeIcon   = d.icon;
    statusText = `${prog.highest_boss || 0}/50 bossů`;
  } else {
    nodeIcon   = '🔒';
    statusText = '🔒 Attunement';
  }

  const progressRing = isUnlocked && !isComplete ? `
    <svg class="dng-node-progress-ring" viewBox="0 0 44 44" xmlns="http://www.w3.org/2000/svg">
      <circle cx="22" cy="22" r="20" fill="none" stroke="${d.color}" stroke-width="2.5"
              stroke-opacity="0.25" stroke-dasharray="125.66" stroke-dashoffset="0"/>
      <circle cx="22" cy="22" r="20" fill="none" stroke="${d.color}" stroke-width="2.5"
              stroke-dasharray="125.66"
              stroke-dashoffset="${125.66 - (progressPct / 100) * 125.66}"
              stroke-linecap="round"
              transform="rotate(-90 22 22)"/>
    </svg>` : '';

  return `
    <div class="dng-map-node ${lockedCls} ${completeCls}"
         style="left:${pos.left};top:${pos.top}"
         onclick="openDngDetail('${d.dungeon_key}')">
      <div class="dng-map-node-ring-wrap">
        <div class="dng-map-node-ring" style="--nc:${d.color};border-color:${d.color}">
          <span class="dng-map-node-icon">${nodeIcon}</span>
          <div class="dng-map-node-pulse" style="--nc:${d.color}"></div>
        </div>
        ${progressRing}
      </div>
      <div class="dng-map-node-label" style="${isUnlocked ? 'color:#fff' : 'color:rgba(255,255,255,.4)'}">
        ${esc(d.name)}
      </div>
      <div class="dng-map-node-status ${!isUnlocked ? 'locked-lbl' : ''}">${statusText}</div>
    </div>`;
}

// ── Zone detail panel ─────────────────────────────────────────────────────────

function openDngDetail(key) {
  ...
  const prog       = _bossProgressMap[key] || {};
  const isUnlocked = prog.is_unlocked !== false;
  const rgb        = _hexToRgb(d.color);
  const progressPct = prog.progress_pct || 0;
  const highestBoss = prog.highest_boss || 0;
  ...
  inner.innerHTML = `
    <div class="dng-detail-banner" style="background:linear-gradient(135deg,${d.bg || 'rgba(0,0,0,.5)'},transparent);border-bottom:1px solid ${d.color}33">
      <div class="dng-detail-icon">${isUnlocked ? d.icon : '🔒'}</div>
    ...
    <div class="dng-detail-body">
      ...
      ${isUnlocked ? `
        <div class="dng-boss-prog-wrap">
          <div class="dng-boss-prog-label">
            <span>Postup v zóně</span>
            <span style="color:${d.color}">${highestBoss} / 50 bossů</span>
          </div>
          <div class="bdc-progress-bar">
            <div class="bdc-progress-fill" style="width:${progressPct}%;background:${d.color}"></div>
          </div>
        </div>` : ''}

      <div class="dng-boss-panel" style="border-color:${d.color}33;background:linear-gradient(135deg,rgba(0,0,0,.35),${d.color}08)">
        <div class="dng-boss-icon">${isUnlocked ? d.boss_icon : '❓'}</div>
        <div class="dng-boss-info">
          <div class="dng-boss-label">Final Boss (Boss #50)</div>
          <div class="dng-boss-name">${isUnlocked ? esc(d.boss_name) : '???'}</div>
        </div>
      ...
      <div class="dng-info-grid" style="grid-template-columns:1fr 1fr;margin-bottom:12px">
        <div class="dng-info-card">
          <span class="dng-info-icon">⚔️</span>
          <span class="dng-info-val">Lv. ${d.min_level}+</span>
          <span class="dng-info-lbl">Min. level</span>
        </div>
        <div class="dng-info-card">
          <span class="dng-info-icon">👹</span>
          <span class="dng-info-val">50</span>
          <span class="dng-info-lbl">Bossů</span>
        </div>
      </div>
      ...
      ${isUnlocked ? `
        ...
        <button class="dng-enter-btn"
                style="--dc-r:${rgb.split(',')[0]};--dc-g:${rgb.split(',')[1]};--dc-b:${rgb.split(',')[2]}"
                onclick="closeDngDetail();openZoneBossView('${d.dungeon_key}')">
          ⚔ Zobrazit bosse ${highestBoss > 0 ? `(${highestBoss}/50)` : ''}
        </button>
      ` : `
